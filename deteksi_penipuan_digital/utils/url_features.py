...
def extract_url_features(url):
    ...
    try:
        ...
        # Deteksi konten judi / gaming berisiko tinggi di URL
        gambling_keywords = [
            'judol', 'slot', 'casino', 'bet', 'betting', 'gambling',
            'jackpot', 'maxwin', 'spin', 'togel', 'poker', 'sbobet'
        ]
        lower_url = url.lower()
        features['is_judol'] = 1 if any(g in lower_url for g in gambling_keywords) else 0
...
def count_suspicious_words(url):
    ...
    suspicious_words = [
        # Fokus ke kata/aksi yang biasanya muncul pada phishing flow.
        # Hindari memasukkan nama brand (paypal/google/facebook/...) karena itu sering muncul pada URL legitimate
        # dan akan meningkatkan false positive.
        'login', 'signin', 'secure', 'account', 'update',
        'verify', 'confirm', 'password', 'reset', 'billing',
        'credit', 'card', 'otp', 'token'
    ]
    count = 0
